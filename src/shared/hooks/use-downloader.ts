import { useWebRTCPeer } from '@/shared/hooks/use-webrtc'
import { useCallback, useEffect, useRef, useState } from 'react'
import { DataConnection } from 'peerjs'
import { ChunkMessage, decodeMessage, Message, MessageType } from '../types'
import { z } from 'zod'
...
const getZipFilename = (): string => `fileeyo-${Date.now()}.zip`
...
export function useDownloader(uploaderPeerID: string): {
  ...
  const startDownload = useCallback(() => {
    ...
    const fileStreams = filesInfo.map((info) => {
      ...
      if (!enqueue || !close) throw new Error('Failed to initialize stream controllers')
